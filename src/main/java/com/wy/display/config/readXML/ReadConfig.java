...
import com.wy.model.decetor.LtpcArea;
import com.wy.model.decetor.LtpcBoard;
...
public class ReadConfig {
    ...
    public static <T> List<T> parseFromExcel(File file, int firstIndex, Class<T> aimClass) {
        ...
        try {
            ...
            for (int i = firstIndex+1; i <= lastRaw; i++) {
                //第i行
    ...
    public static void setDetectorByXlxs(File file) throws Exception {
        ...
        List<LtpcChannel> listChannels = parseFromExcel(file, 0, LtpcChannel.class);
        listChannels.forEach(c -> {
            c.setSourceBoardNum(getSourecBoardNum(c.getArea(),c.getBoard()));
        });
        int board=-1,area=-1;
